...
import { createQuiz, getQuizzesByUser } from "@/services/quizService";
...
interface UserProfile {
  userId: string;
  email?: string;
  // Add other fields as needed for HeaderProfile
}
      ...
      try {
        ...
        if (profileData?.userId) {
          ...
          const fetchPromises = statuses.map(async (status) => {
            const quizzesData = await getQuizzesByUser(profileData.userId, pagination[status].currentPage, pageSize, status);
    ...
    <div className="container-xxl flex-grow-1 container-p-y">
      ...
      <div className="row">
        <div className="col-xl-12 d-flex justify-content-end">
          <CreateQuizButtonWithModal profile={profile} disabled={creatingQuiz} />
        </div>
                    ...
                    {pagination.DRAFT.totalPages > 1 && (
                      <nav aria-label="Draft quizzes pagination" className="mt-4">
                        <ul className="pagination justify-content-center">
                          ...
                          <li
                            className={`page-item ${
                              pagination.DRAFT.currentPage === pagination.DRAFT.totalPages - 1 ? "disabled" : ""
                            }`}
                          >
                            <button
                              className="page-link"
                    ...
                    {pagination.CLOSED.totalPages > 1 && (
                      <nav aria-label="Archived quizzes pagination" className="mt-4">
                        <ul className="pagination justify-content-center">
                          ...
                          <li
                            className={`page-item ${
                              pagination.CLOSED.currentPage === pagination.CLOSED.totalPages - 1 ? "disabled" : ""
                            }`}
                          >
                            <button
                              className="page-link"
