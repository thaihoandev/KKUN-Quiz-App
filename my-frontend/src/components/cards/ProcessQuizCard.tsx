...
import { handleApiError } from "@/utils/apiErrorHandler"; // Adjust path as needed
...
const ProcessQuizCard = ({ quiz }: QuizCardProps) => {
  ...
  const displayDescription = quiz.description || `A quiz hosted by ${quiz.host.name}. Explore to test your knowledge!`;
  ...
  // Fallback avatar
  const displayAvatar = quiz.host.avatar || "../../assets/img/pages/app-academy-tutor-1.png";
      ...
      <div className="card p-2 h-100 shadow-none border">
        <div className="card-body p-4 pt-2">
          ...
          <div className="d-flex justify-content-between align-items-center mb-4">
            <span className={`badge bg-label-${getCategoryColor(quiz.category)}`}>
              {displayCategory}
            </span>
            <p className="d-flex align-items-center justify-content-center fw-medium gap-1 mb-0">
              ...
              <span className="fw-normal">({quiz.viewers?.length || 0})</span>
            </p>
          </div>
          <a href={`/quizzes/${quiz.quizId}`} className="h5">
            {quiz.title}
          </a>
          ...
          <div className="d-flex flex-column flex-md-row gap-4 text-nowrap flex-wrap flex-md-nowrap flex-lg-wrap flex-xxl-nowrap">
            <button
              ...
              disabled={isStarting}
            >
              <span className="me-2">{isStarting ? "Starting..." : "Start Quiz"}</span>
